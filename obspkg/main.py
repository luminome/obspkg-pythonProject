...
import obspkg_tests as utl_tst
...
def builder(cfg):
    ...
    if "sst" in cfg["includes"]:
        ...
        hi_res_sst = utl_tst.get_hi_res_sst_data()
        value_limits = {
            'min': utl_tst.k_to_deg_c(np.nanmin(hi_res_sst['data'])),
            'max': utl_tst.k_to_deg_c(np.nanmax(hi_res_sst['data']))
        }
        print("value_limits", value_limits)

        cfg['includes_attributes']['sst'] = {
            "valid": True,
            "levels": 4,
            "time": True,
            "time-indices": hi_res_sst['time'].shape[0],
            "dimension": 2,
            "limits": value_limits
        }

        cfg['includes_data']['sst'] = hi_res_sst

        times_index = [get_time_as_dict(t, 's', '1981-01-01 00:00:00') for t in hi_res_sst['time']]
        d_path = deliver(times_index, session_path, 'sst-indices')
        print(f"Saved sst index data to: '{d_path}'")

    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if "wind" in cfg["includes"]:
        print('wind (surface wind)')
        lo_res_wind = utl_tst.get_lo_res_wind_data()

        cfg['includes_attributes']['wind'] = {
            "valid": True,
            "levels": 4,
            "time": True,
            "time-indices": lo_res_wind['time'].shape[0],
            "dimension": 2
        }

        cfg['includes_data']['wind'] = lo_res_wind

        times_index = [get_time_as_dict(t, 'h') for t in lo_res_wind['time']]
    ...
    for i, sector_check in enumerate(sector_group):
        ...
        if "urban" in cfg["includes"]:
            ...
            for ub in urban_areas:
                #print(ub)
                if ub['poly'].type == 'MultiPolygon':
                    ub['poly'] = [utl.coords_to_flat(p.exterior.coords) for p in ub['poly'].geoms]
                else:
        ...
        if "sst" in cfg["includes"]:
            main_sst_data = cfg['includes_data']['sst']
            levels_and_times = utl_tst.get_sector_sst(main_sst_data, cfg['map_degrees'], sector_check.tuple)
            levels_and_times.reverse()
            ref = [len(p[0]) for p in levels_and_times]
            cfg['includes_attributes']['sst']['resolutions'] = ref
            cfg['includes_attributes']['sst']['levels'] = len(ref)
            #//print(ref)

            for lv, gl in enumerate(levels_and_times):
                datum = {"indx": main_sst_data['time'].tolist(), "data": gl}
                sector_check.add_data("sst", lv, datum, {"d": 2, "indx": len(gl)})

            sector_check.save("sst", "data_levels")

        # #//ADD WIND at every level IF SO DOING
        if "wind" in cfg["includes"]:
            main_wind_data = cfg['includes_data']['wind']
            levels_and_times = utl_tst.get_sector_wind(main_wind_data, cfg['map_degrees'], sector_check.tuple)
            levels_and_times.reverse()

            ref = [len(p[0]) for p in levels_and_times]
            cfg['includes_attributes']['wind']['resolutions'] = ref
            cfg['includes_attributes']['wind']['levels'] = len(ref)
            #print(ref)

            for lv, gl in enumerate(levels_and_times):
                #//print(gl)
                datum = {"indx": main_wind_data['time'].tolist(), "data": gl}
                sector_check.add_data("wind", lv, datum, {"d": 2, "indx": len(gl)})
