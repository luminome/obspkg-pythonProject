...
if __name__ == '__main__':
    ...
    sectors = [sector for sector in map(graze, [(10, 2.5, 2), (12, 2.5, 2)])]

    def make_contours(data):
        contours = measure.find_contours(data, -120.0)
        def make_poly(contour):
            poly = Polygon(np.flip(contour))
            #poly = poly.simplify(4.0)
            return poly

        return map(make_poly, contours)

    polygons = [p for p in make_contours(dat)]

    def do_refine(shape):
        x, y = shape.coords.xy
        tck, u = interpolate.splprep([x, y], s=4, k=3)  #, k=0)
        u_new = np.arange(0, 1.01, 0.01)
        out = interpolate.splev(u_new, tck)
        plt.plot(out[0], out[1], linewidth=1, color=(1.0, 1.0, 1.0))
        # plt.plot(x, y, out[0], out[1], linewidth=1, color=(1.0, 1.0, 1.0))

        # tout = b_spline_to_bezier_series(tck, True)
        # sx, sy = tout[:, 0], tout[:, 1]
        # plt.plot(sx, sy, 'ro', color=(0.0, 1.0, 1.0)) #, linewidth=0.25)
