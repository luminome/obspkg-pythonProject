...
if __name__ == '__main__':
    ...
    gauss_limits = [3.0, 0.0]
    simp_limits = [0.025, 0.0005]
    interval_limits = [6, 12]

    # areas = np.linspace(area_limits[0], area_limits[1], 4)
    # simps = np.linspace(simp_limits[0], simp_limits[1], 4)

    contour_sets_per_level = []

    def draw_contours_for_level(level, data, gauss, simplification, interval):
        dlim = np.arange(1, 4000, interval)
        g_data = gaussian_filter(data, sigma=.5)

        contours = measure.find_contours(g_data, 1)


        pass
        return  #all_contours_at_depths_for_level
